...
## 3. 분석 내용
### 3-1. 기존 연구
1. 시계열 데이터를 Prophet 알고리즘을 사용해 코로나19 동향 예측
...
- 0.914의 결정 계수를 가짐
2. 코로나 사태가 국내 경제에 미치는 영향과 향후 과제
...
- 코로나, 경제 데이터 회귀 분석
  ...
  - 코로나 이전의 경제 동향을 확인하기 위해 2018년 부터 데이터를 적용했으나 코로나 발생 전 까지의 모든 코로나 데이터 컬럼이 0 값으로 입력되어 모델 성능에 영향을 주고 결정 계수가 무의미하게 높게 나오는 것을 확인, 코로나 국내 발생 일자에 따라 2020-01-20 부터 시작으로 데이터 적용
  - 최소 제곱법(OLS)
